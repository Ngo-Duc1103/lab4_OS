	...
	while (!load_done || !empty(&ready_queue)) {
		...
		if (proc == NULL) {
			...
			*/
			...
			// YOUR CODE HERE
			waitingtime += (timestamp - proc->lasttime);
			if (proc->burst_time < timeslot) { 
				exec_time = proc->burst_time;
				proc->burst_time = 0;
				// proccess while be deleted so ignore priority
			} else { 
				exec_time = timeslot;
			...
			// YOUR CODE HERE
		  	if (!(proc->burst_time)) {
				...
				if (proc->priority) proc->priority--;
...
void * loader(void * arg) {
	...
	while (!empty(&in_queue)) {
    	...
    	if (0 < wastetime) usleep(wastetime * TIME_UNIT);
...
void load_task() {
	...
	for (i = 0; i < num_proc; i++) {
		...
		proc->lasttime = proc->arrival_time;
		//printf("First time of %d is %d\n", i, proc->lasttime);
		process++;
		proc->pid = i;
		en_inqueue(&in_queue, proc);
	}
}
